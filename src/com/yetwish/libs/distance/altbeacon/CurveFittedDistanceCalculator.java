...
public class CurveFittedDistanceCalculator implements DistanceCalculator {
    ...
    private static int calculateTimes = 5;

    public static void setCalculateTimes (int times){
    ...
    public double calculateDistance(int txPower, double rssi) {
        ...
        double averageDistance = Constants.INIT_DISTANCE;
        double sum = 0 ;
        for (int i =0 ;i< calculateTimes;i++){
            sum += getOneTimeCalculation(txPower,rssi);
        }
        averageDistance = sum /calculateTimes;
        return averageDistance;
        //LogManager.d(TAG, "calculating distance based on mRssi of %s and txPower of %s", rssi, txPower);
    }

    private double getOneTimeCalculation(int txPower,double rssi){
        //LogManager.d(TAG, "calculating distance based on mRssi of %s and txPower of %s", rssi, txPower);
